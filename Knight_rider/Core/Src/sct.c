/*
 * sct.c
 *
 *  Created on: Oct 12, 2023
 *      Author: xvever12
 */

#include "sct.h"
#include <math.h>

void sct_init(void)
{
	sct_led(0);
}

void sct_led(uint32_t value)
{
	for (int i = 0; i < 32; i++){
		HAL_GPIO_WritePin(SCT_SDI_GPIO_Port, SCT_SDI_Pin, (value>>i)%2);

		//CLK PULS
		HAL_GPIO_WritePin(SCT_CLK_GPIO_Port, SCT_CLK_Pin, 1);
		HAL_GPIO_WritePin(SCT_CLK_GPIO_Port, SCT_CLK_Pin, 0);
	}

	HAL_GPIO_WritePin(SCT_NLA_GPIO_Port, SCT_NLA_Pin, 1);
	HAL_GPIO_WritePin(SCT_NLA_GPIO_Port, SCT_NLA_Pin, 0);

	HAL_GPIO_WritePin(SCT_NOE_GPIO_Port, SCT_NOE_Pin, 0);

}

//3 displeje a LED (V případě displeje tak "P" je tečka

static const uint32_t reg_values[4][10] = {
	{
	//PCDE--------GFAB @ DIS1
	0b0111000000000111 << 16,
	0b0100000000000001 << 16,
	0b0011000000001011 << 16,
	0b0110000000001011 << 16,
	0b0100000000001101 << 16,
	0b0110000000001110 << 16,
	0b0111000000001110 << 16,
	0b0100000000000011 << 16,
	0b0111000000001111 << 16,
	0b0110000000001111 << 16,
	},
	{
	//----PCDEGFAB---- @ DIS2
	0b0000011101110000 << 0,
	0b0000010000010000 << 0,
	0b0000001110110000 << 0,
	0b0000011010110000 << 0,
	0b0000010011010000 << 0,
	0b0000011011100000 << 0,
	0b0000011111100000 << 0,
	0b0000010000110000 << 0,
	0b0000011111110000 << 0,
	0b0000011011110000 << 0,
	},
	{
	//PCDE--------GFAB @ DIS3
	0b0111000000000111 << 0,
	0b0100000000000001 << 0,
	0b0011000000001011 << 0,
	0b0110000000001011 << 0,
	0b0100000000001101 << 0,
	0b0110000000001110 << 0,
	0b0111000000001110 << 0,
	0b0100000000000011 << 0,
	0b0111000000001111 << 0,
	0b0110000000001111 << 0,
	},
	//----43215678---- @ LED
	{
	0b0000000100000000 << 16,
	0b0000001100000000 << 16,
	0b0000011100000000 << 16,
	0b0000111000000000 << 16,
	0b0000110010000000 << 16,
	0b0000100011000000 << 16,
	0b0000000011100000 << 16,
	0b0000000001110000 << 16,
	0b0000000000110000 << 16,
	0b0000000000010000 << 16,
	},
};

// if pointIndex == 100 -> set point after first digit from left (1.00)
// if pointInted == 10 -> set point after second digit from left (10.0)
void sct_value(int16_t value, uint8_t led)
{
	uint32_t reg = 0;

		reg |= reg_values[0][value / 100 % 10];
		reg |= reg_values[1][value / 10 % 10];
		reg |= reg_values[2][value / 1 % 10];

		reg |= reg_values[3][led];



	sct_led(reg);
}


//KOD pro displeje pro čísla i s 10 čárkou případně nápis
/*void sct_value(float f_value, uint8_t led)
{
	uint32_t reg = 0;
	uint16_t value = 0;


	f_value = 8.54;

	value = f_value * 100;

	reg |= reg_values[0][value / 100 % 10];
	reg |= reg_values[1][value / 10 % 10];
	reg |= reg_values[2][value / 1 % 10];

	if (f_value < 10)
		reg |= 0b1000000000000000 << 16; // 1.00
	else if (f_value < 100)
		reg |= 0b0000100000000000 << 0; // 10.0

	reg |= reg_values[3][led];

	sct_led(reg);
}

//KOD pro písmena
void say_hi()
{
	uint32_t reg = 0;

	reg |= reg_values[0][0]; // H
	reg |= reg_values[1][0]; // I

	sct_led(reg);

}
 *
 */
